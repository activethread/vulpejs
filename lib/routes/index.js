...
exports.auth = {
  ...
  check: function (req, res, next) {
    ...
    if (!req.isAuthenticated()) {
      ...
      var uriCheck = function (value) {
        return (value === '/' && req.originalUrl === value) || (value !== '/' && req.originalUrl.indexOf(value) === 0);
      };
      for (var i = 0; i < routes; ++i) {
        var route = vulpejs.app.security.routes[i];
        if (route.uris && Array.isArray(route.uris)) {
          for (var j = 0; j < route.uris.length; ++j) {
            if (uriCheck(route.uris[j])) {
...
exports.make = function (options) {
    ...
    if (options.headers) {
      for (var header in options.headers) {
        ...
        if (!req.headers[value]) {
          res.json({
            success: false,
            message: vulpejs.i18n.__('Request header parameter not found: %s.', value)
          }).status(400);
          return;
        }
      }
    }
    vulpejs.utils.execute(next);
  };
  // VIEW
  ...
  var configure = function (req, type) {
    ...
    if (type && type === 'SEARCH') {
      ...
      for (var property in options.list.query) {
        var value = options.list.query[property];
        if (value && value.header) {
          options.list.query[property] = req.headers[value.header];
        }
      }
      if (type && type === 'STATUS') {
  ...
  router.get('/' + options.name + '/status/:status', function (req, res) {
    headers(req, res, function () {
      configure(req, 'STATUS');
      exports.list.do(req, res);
    });
  });
  router.get('/' + options.name + '/search/:search', function (req, res) {
    headers(req, res, function () {
  ...
  router.get('/' + options.name + '/page/:page', function (req, res) {
    headers(req, res, function () {
      configure(req);
      exports.paginate.do(req, res);
    });
  });
  router.get('/' + options.name + '/status/:status/page/:page', function (req, res) {
    headers(req, res, function () {
      configure(req, 'STATUS');
      exports.paginate.do(req, res);
    });
  });
  router.get('/' + options.name + '/search/:search/page/:page', function (req, res) {
    headers(req, res, function () {
      configure(req, 'SEARCH');
      exports.paginate.do(req, res);
    });
...
exports.init = function () {
    ...
    vulpejs.io.read.dir(vulpejs.app.root.dir + '/public/javascripts/internal/controllers/', function (list) {
      ...
      if (list) {
        vulpejs.async.eachSeries(list, function (name, callback) {
          ...
          vulpejs.io.read.dir(vulpejs.root.dir + '/ui/public/javascripts/internal/controllers/', function (list) {
            if (list) {
              vulpejs.async.eachSeries(list, function (name, callback) {
                controllers.push(vulpejs.io.read.file(vulpejs.root.dir + '/ui/public/javascripts/internal/controllers/' + name));
                callback();
              }, function () {
  ...
  if (!vulpejs.app.backend) {
    router.post('/filemanager/upload', function (req, res) {
      ...
      form.multiples = true;
      form.uploadDir = vulpejs.app.uploader[vulpejs.app.env].dir.files;
      var fileUploaded = {};
      form.on('file', function (field, file) {
        fileUploaded = file;
        vulpejs.io.file.rename(file.path, path.join(form.uploadDir, file.name));
      });
    ...
    router.get('/filemanager/download/:name', function (req, res) {
      var file = vulpejs.app.uploader[vulpejs.app.env].dir.files + '/' + req.params.name;
      res.download(file);
    });

    router.delete('/filemanager/remove/:name', function (req, res) {
      var file = vulpejs.app.uploader[vulpejs.app.env].dir.files + '/' + req.params.name;
      vulpejs.io.file.remove(file, function () {
        res.json({
          success: true
        });
      });

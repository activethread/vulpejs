...
exports.doLoginJWT = function(req, res, next) {
  vulpejs.express.passport.authenticate('jwt', function(error, user, info) {
    if (error) {
      vulpejs.utils.execute(vulpejs.app.callback.login.error, {
        user: user,
        error: error
    ...
    if (!user) {
      ...
      return res.redirect(401, '/login-jwt');
    }
...
exports.init = function(options) {
  ...
  var token = jwt.sign({
    email: 'admin@sitehosting.com.br'
  }, 'secret');
  console.log(token);
  ...
  router.post('/login-jwt', vulpejs.express.passport.authenticate('jwt', {
      session: false
    }),
    function(req, res) {

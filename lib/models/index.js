...
exports.init = function(options) {
  ...
  if (!options.database) {
    ...
    if (!options.database.auth.source) {
      options.database.auth.source = 'admin';
    }
    if (!options.database.auth.user) {
      options.database.auth.user = 'admin';
    }
    if (!options.database.auth.pass) {
      options.database.auth.pass = 'vulpejs';
    }
  }

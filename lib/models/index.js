        ...
        if (error) {
          ...
          var save = function() {
            ...
            oldItem.save(function(error, obj) {
              callback(obj, error);
              if (error) {
                vulpejs.log.error('ITEM-UPDATE-ERROR', error);
                vulpejs.log.error('ITEM-UPDATE-OPTIONS', options);
                vulpejs.utils.execute(options.callback.error);
              } else {
...
exports.init = function() {
  ...
  vulpejs.mongoose.connect('mongodb://' + auth + '' + database.host + ':' + database.port + '/' + database.name + '?' + authSource + 'w=1');

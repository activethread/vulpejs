 ...
 */
app.factory('VulpeJS', ['$rootScope', '$parse', '$http', '$authenticator', '$messages', '$dialogs', '$timeout', '$interval', 'i18n', '$store', '$cookieStore', '$cookies', '$sce', '$window', '$filter', function($rootScope, $parse, $http, $authenticator, $messages, $dialogs, $timeout, $interval, i18n, $store, $cookieStore, $cookies, $sce, $window, $filter) {
    ...
    watch: function(value, callback) {
      if (angular.isArray(value)) {
        value.forEach(function(property) {
          $rootScope.$watch(property, callback);
        });
      } else if (angular.isObject(value) && value.item && value.properties) {
        value.properties.forEach(function(property) {
          $rootScope.$watch(value.item + '.' + property, callback);
        });
      } else {
        $rootScope.$watch(value, callback);
      }
    },
    filter: {
      ...
      confirm: function(options) {
        $dialogs.confirm(vulpejs.i18n('Confirmation'), vulpejs.i18n(options.message)).result.then(function(btn) {
          if (options.callback.yes) {
            vulpejs.utils.execute(options.callback.yes, btn);
          } else {
            vulpejs.utils.execute(options.callback, btn);
          }
        }, function(btn) {
      ...
      get: function(options) {
        $http({
          ...
          if (options.callback.success) {
            ...
            vulpejs.utils.execute(options.callback, data);
          };
      ...
      uploader: {
        ...
        flow: {
          ...
          submit: function($flow, model) {
            ...
            vulpejs.ui.uploader.flow.submitBefore($flow, $model, model);
            $flow.upload();
            vulpejs.ui.uploader.flow.submitAfter($flow, $model, model);
          },
          successBefore: nothing,
          successAfter: nothing,
          success: function($flow, $file, model) {
            var $model = $parse(model);
            vulpejs.ui.uploader.flow.successBefore($flow, $file, $model, model);
            $model.assign($rootScope, window.location.origin + '/flow/download/' + $file.uniqueIdentifier + '?' + Date.now());
            vulpejs.ui.uploader.flow.successAfter($flow, $file, $model, model);
          },
          cancelBefore: nothing,
          cancelAfter: nothing,
  ...
  service.prototype.init = function($scope) {
    ...
    vulpejs.utils.execute(application.init);
    if ($scope) {
      $(document).ready(function() {

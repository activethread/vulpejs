...
/* Services */
angular.module('app.services', ['ui.bootstrap', 'ui.router', 'dialogs', 'i18n', 'ngCookies', 'ngResource']).config(function($stateProvider, $urlRouterProvider) {
...
}]).factory('VulpeJS', ['$rootScope', '$parse', '$http', '$authenticator', '$messages', '$dialogs', '$timeout', '$interval', 'i18n', '$store', '$cookieStore', '$cookies', '$sce', '$window', '$filter', '$resource', function($rootScope, $parse, $http, $authenticator, $messages, $dialogs, $timeout, $interval, i18n, $store, $cookieStore, $cookies, $sce, $window, $filter, $resource) {
    ...
    http: {
      ...
      post: function(options) {
        $http.post(options.url, options.data).then(function(response) {
          vulpejs.utils.execute(options.callback.success, response.data);
        }, function(response) {
          vulpejs.utils.execute(options.callback.error, response);
        });
      },
  ...
  var service = function(options) {
    if (options) {
      ...
      if (options.messages) {
        if (options.messages.save) {
          vulpejs.model.messages.save = options.messages.save;
        }
        if (options.messages.remove) {
          vulpejs.model.messages.remove = options.messages.remove;
        }
        if (options.messages.clone) {
          vulpejs.model.messages.clone = options.messages.clone;
        }
        if (options.messages.validate) {
          vulpejs.model.messages.validate = options.messages.validate;
        }
      }
        ...
        if ($scope) {
          $(document).ready(function() {
            for (var form in vulpejs.ui.forms) {
              vulpejs.ui.form[form] = $scope[form + 'Form'];
            }

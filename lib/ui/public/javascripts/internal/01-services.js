...
/* Services */
angular.module('app.services', ['ui.bootstrap', 'ui.router', 'dialogs.main', 'i18n', 'ngCookies', 'ngResource']).config([
  ...
  var vulpejs = {
    ...
    model: {
      ...
      find: function (id) {
        ...
        $http({
          url: vulpejs.url.get() + (vulpejs.model.populate ? '/populate' : '') + '/' + id,
          method: 'GET',
          headers: vulpejs.now.headers || {}
        }).then(function (response) {
          var data = response.data;
      ...
      list: function (options) {
        ...
        if (!vulpejs.ui.form[options.form].$valid) {
          vulpejs.ui.form.validate(options.form);
          return;
        }
        if (!options.page) {
          options.page = 1;
        ...
        $http({
          url: vulpejs.url.get(vulpejs.url.list) + filter,
          method: 'GET',
          headers: vulpejs.now.headers || {}
        }).then(function (response) {
          var data = response.data;
      ...
      save: function (options) {
        ...
        if (options.type) {
          vulpejs.model.saveType = options.type;
          return;
        }
        if (vulpejs.item._id) {
        ...
        array: function (options) {
          ...
          if (from.length > 0) {
            $http({
              ...
              vulpejs.utils.execute(callback, $rootScope.vulpejs);
            });
          }
        },
    ...
    ui: {
      ...
      form: {
        ...
        validate: function (form) {
          var error = vulpejs.ui.form[form].$error;
          if (error && error.required) {
            for (var i = 0; i < error.required.length; ++i) {
              var input = error.required[i];
              var controller = vulpejs.ui.name.replace(/\-/g, '');
              var id = controller + '-' + form + '-' + input.$name;
              var element = vulpejs.ui.get(id);
              if (element.length === 1) {
                var tabindex = $(element).attr('tabidx');
                if (tabindex) {
                  vulpejs.ui.tab[controller] = parseInt(tabindex);
                }
                vulpejs.timeout.add(function () {
                  element.focus();
                });
      ...
      uploader: {
        ...
        remove: function (name, file) {
          vulpejs.http['delete']({
            url: '/filemanager/remove/' + file,
            params: {},
            callback: function () {
              var $model = $parse('vulpejs.item.' + name);
          ...
          if (files.length > 0) {
            ...
            $.ajax({
              ...
              success: function (data) {
                var $model = $parse('vulpejs.item.' + vulpejs.now[id]);
                $model.assign($rootScope, data.file.name);
                vulpejs.model.save({
                  ignore: true
                });
                delete vulpejs.now[id];
              },
              xhr: function () {
                ...
                xhr.upload.addEventListener('progress', function (evt) {
                  if (evt.lengthComputable) {
                    ...
                    if (percentComplete === 100) {
                      $progress.html(vulpejs.i18n('Done'));
                    }
                  }
                }, false);
                return xhr;
          ...
          vulpejs.timeout.add(function () {
            if (value) {
              ...
              if (angular.isFunction(options.focus)) {
                ...
                var prefix = '#' + options.name.replace(/\-/g, '') + '-' + (vulpejs.operation === 'LIST' ? 'items' : 'main') + '-';

...
module.exports = function(opts) {
  var path = require('path'),
    ...
    options = {
      ...
      downloadUrl: opts.downloadUrl || '/files/',
  ...
  var FileInfo = function(file) {
    this.name = utils.removeDiacritics(file.name.replace(/\s/g, '-').replace(/\[|\]|\(|\)/g, ''));
    this.size = file.size;
    this.type = file.type;
  ...
  FileInfo.prototype.initUrls = function(req, sss) {
    if (!this.error) {
      ...
      if (!sss) {
        ...
        if (that.name.match(/(\.|\/)(mp4|mov)$/i)) {
          that.preview = {
            type: 'video',
            src: that.downloadUrl
          };
        }
      } else {
        that.url = sss.url;
        that.deleteUrl = options.uploadUrl + sss.url.split('/')[sss.url.split('/').length - 1].split('?')[0];
        that.downloadUrl = options.downloadUrl + sss.url.split('/')[sss.url.split('/').length - 1].split('?')[0];
